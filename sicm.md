...
This is close. To get the result we want, WOAH, set \\(\phi = 0\\):
...
\begin{equation}
\begin{pmatrix} \displaystyle{  - m {\dot{\phi}}^{2} r + m \ddot{r} + DU\left( r \right)} \cr \cr \displaystyle{ 2 m \dot{\phi} \dot{r} + m \ddot{\phi} r}\end{pmatrix}
\end{equation}

But there is no justification for that. So instead, I stared at this for a while&#x2026; and eventually noticed this linear combination that gets me to the result.
